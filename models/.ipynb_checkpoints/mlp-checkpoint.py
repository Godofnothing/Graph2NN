...
class LinearTransform(nn.Module):
    ...
    def __init__(self, cfg, dim_in, dim_out, seed=None):
        super(LinearTransform, self).__init__()
        self._construct_class(cfg, dim_in, dim_out)

    def _construct_class(self, cfg, dim_in, dim_out):
        # 3x3, BN, ReLU
        self.a = nn.Linear(
...
class TalkLinearTransform(nn.Module):
    ...
    def _construct_class(self, cfg, dim_in, dim_out):
        self.a = TalkLinear(
            ...
            graph_type=cfg["RGRAPH"]["GRAPH_TYPE"], sparsity=cfg["RGRAPH"]["SPARSITY"],
...
class MLPStage(nn.Module):
    ...
    def __init__(self, cfg, dim_in, dim_out, num_bs):
        super(MLPStage, self).__init__()
        self._construct_class(cfg, dim_in, dim_out, num_bs)

    def _construct_class(self, cfg, dim_in, dim_out, num_bs):
        ...
        for i in range(num_bs):
            ...
            res_block = trans_fun(
                cfg, b_dim_in, dim_out, seed=seed
            )
...
class MLPStem(nn.Module):
    ...
    def __init__(self, cfg, dim_in, dim_out):
        ...
        if cfg["TRAIN"]["DATASET"] in ['cifar10', 'mnist']:
            self._construct(cfg, dim_in, dim_out)
        else:
            raise NotImplementedError

    def _construct(self, cfg, dim_in, dim_out):
...
class MLPNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg["TRAIN"]["DATASET"] == 'cifar10':
            self._construct_cifar(cfg)
        elif cfg["TRAIN"]["DATASET"] == 'mnist':
            self._construct_mnist(cfg)
        else:
            raise NotImplementedError
    ...
    def _construct_mnist(self, cfg):
        num_layers = cfg["MODEL"]["LAYERS"]
        dim_inner = cfg["RGRAPH"]["DIM_LIST"][0]
        dim_first = cfg["RGRAPH"]["DIM_FIRST"]
        self.s1 = MLPStem(cfg, dim_in=28 * 28, dim_out=dim_first)
        self.s2 = MLPStage(cfg, dim_in=dim_first, dim_out=dim_inner, num_bs=num_layers)
        self.head = MLPHead(dim_in=dim_inner, num_classes=cfg["MODEL"]["NUM_CLASSES"])
