...
from src.relation_graph import *
...
class ConvBasicTransform(nn.Module):
    ...
    def __init__(self, cfg, dim_in, dim_out, stride, dim_inner=None, num_gs=1, seed=None):
        super(ConvBasicTransform, self).__init__()
        self._construct_class(dim_in, dim_out, stride)

    def _construct_class(self, cfg, dim_in, dim_out, stride):
        # 3x3, BN, ReLU
        self.a = nn.Conv2d(
...
class ConvTalkTransform(nn.Module):
    ...
    def __init__(self, cfg, dim_in, dim_out, stride, dim_inner=None, num_gs=1, seed=None):
        ...
        self._construct_class(cfg, dim_in, dim_out, stride)
    ...
    def _construct_class(self, cfg, dim_in, dim_out, stride):
        ...
        self.a = TalkConv2d(
            ...
            stride=stride, padding=1, bias=False,
            graph_type=cfg["RGRAPH"]["GRAPH_TYPE"], agg=cfg["RGRAPH"]["AGG_FUNC"],
...
class CNNStage(nn.Module):
    ...
    def __init__(self, cfg, dim_in, dim_out, stride, num_bs, dim_inner=None, num_gs=1):
        super(CNNStage, self).__init__()
        self._construct_class(cfg, dim_in, dim_out, stride, num_bs, dim_inner, num_gs)

    def _construct_class(self, cfg, dim_in, dim_out, stride, num_bs, dim_inner, num_gs):
        ...
        for i in range(num_bs):
            ...
            # Construct the block
            res_block = trans_fun(
                cfg, b_dim_in, dim_out, b_stride, dim_inner, num_gs, seed=seed
            )
...
class CNNStem(nn.Module):
    ...
    def __init__(self, cfg, dim_in, dim_out):
        ...
        if cfg["TRAIN"]["DATASET"] == 'cifar10':
            self._construct_cifar(cfg, dim_in, dim_out)
        else:
            self._construct_mnist(cfg, dim_in, dim_out)

    def _construct_cifar(self, cfg, dim_in, dim_out):
        # 3x3, BN, ReLU
    ...
    def _construct_mnist(self, cfg, dim_in, dim_out):
        # 3x3, BN, ReLU
        self.conv = nn.Conv2d(
                dim_in, dim_out, kernel_size=3,
                stride=1, padding=1, bias=False
